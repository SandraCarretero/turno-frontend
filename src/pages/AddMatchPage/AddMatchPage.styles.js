...
export const CooperativeResultOption = styled.label`
  ...
  span {
    font-weight: 500;
    color: ${props =>
      props.$success ? '#28a745' : props.$error ? '#dc3545' : 'inherit'};
  }
`;
...
export const Button = styled.button`
  ...
  padding: ${props =>
    props.$variant === 'link' ? '0.25rem 0.5rem' : '1rem 2rem'};
  ...
  &:hover:not(:disabled) {
    ...
    transform: ${props =>
      props.$variant === 'link' ? 'none' : 'translateY(-2px)'};
    box-shadow: ${props =>
      props.$variant === 'link' ? 'none' : '0 4px 12px rgba(0, 123, 255, 0.3)'};
  }
