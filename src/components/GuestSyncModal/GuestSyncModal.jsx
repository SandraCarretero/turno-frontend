...
const GuestSyncModal = ({ isOpen, onClose, guestId, guestData, onSyncComplete }) => {
        ...
        <ModalBody>
          {loading && !syncSuccess ? (
            ...
            <SyncSuccess>
              ...
              <p>
                {guest?.name} ha sido sincronizado con {selectedUser?.username}. Ahora las estadísticas y partidas se
                compartirán entre ambos perfiles.
              </p>
            </SyncSuccess>
          ) : (
            <>
                  ...
                  <UserItem $noHover>
                    ...
                    <UserInfo>
                      ...
                      <p style={{ margin: 0, fontSize: "0.85rem", color: "#6c757d" }}>
                        {guestId ? `${guest.gamesPlayed || 0} partidas jugadas` : "Nuevo invitado"}
                      </p>
                    </UserInfo>
                  </UserItem>
                </div>
              ...
              <p style={{ margin: "0 0 1rem 0", fontSize: "0.9rem", color: "#6c757d" }}>
                Busca un usuario registrado para sincronizar con este invitado. Las estadísticas y partidas se
                compartirán entre ambos perfiles.
              </p>
              ...
              {searching && <p style={{ fontSize: "0.9rem", color: "#6c757d" }}>Buscando usuarios...</p>}

              {users.length > 0 ? (
                <UserList>
                  {users.map((user) => (
                    <UserItem key={user._id} onClick={() => handleSyncUser(user)}>
